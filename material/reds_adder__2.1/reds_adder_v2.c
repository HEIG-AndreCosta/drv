...
/*
 * REDS-adder driver, v2.1, 20.07.2022
 *
 * SAME AS v1.1, but added a couple of sysfs entries for test purposes.
 *
 */
			       ...
			       size_t count)
{
	...
	dev_info(priv->dev, "Just to be sure, the IRQ number is %d\n",
		 priv->irq_num);
	dev_info(priv->dev, "Received new threshold = %d, rc = %d\n", tmp, rc);
...
/**
 * @brief Display the current threshold.
 *
 * @param dev: pointer to our device
 * @param attr: pointer to the associated attributes (ignored)
			      ...
			      char *buf)
{
	...
	dev_info(priv->dev, "Just to be sure, the IRQ number is %d\n",
		 priv->irq_num);
	dev_info(priv->dev, "Returned value = %d, rc = %d\n", tmp, rc);
			    ...
			    loff_t *ppos)
{
	...
	if (count > MAX_VEC_LEN) {
		dev_err(priv->dev,
			"%s(): cannot ask for more than buffer size !\n",
			__func__);
		return 0;
	}
...
static int reds_adder_probe(struct platform_device *pdev)
{
	...
	/* Create the sysfs entry for the threshold. */
	rc = device_create_file(&pdev->dev, &dev_attr_threshold);
...
static int reds_adder_remove(struct platform_device *pdev)
{
	...
	/* Remove the sysfs entry for the threshold. */
	device_remove_file(priv->dev, &dev_attr_threshold);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("REDS");
MODULE_DESCRIPTION("REDS-adder driver v2.1");
